...
public Show(id)
{
	...
	if(get_user_team(cel) == get_user_team(id))
	{
		...
		if(get_pcvar_num(team_show_nick) == 1)
			Len += formatex(txt, (sizeof txt - 1) - Len, "%L: %s^n", id, "NAME", nazwa_gracza)
		if(get_pcvar_num(team_show_health) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", id, "HEALTH", get_user_health(cel))
		if(get_pcvar_num(team_show_level) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", id, "LEVEL", exp_get_user_level(cel))
		if(get_pcvar_num(team_show_exp) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", id, "EXP", exp_get_user_exp(cel))
		if(get_pcvar_num(team_show_mission) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %s^n", id, "MISSION", nazwa_misji)
