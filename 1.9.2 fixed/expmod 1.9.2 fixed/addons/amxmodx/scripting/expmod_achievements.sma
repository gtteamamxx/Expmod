...
public plugin_init() 
{
	...
	plikI 	= nvault_open("exp_achI")
	plikII 	= nvault_open("exp_achII")
	plikIII = nvault_open("exp_achIII")
	
	plikI_ 	= nvault_open("exp_achI_")
	plikII_ = nvault_open("exp_achII_")
	plikIII_= nvault_open("exp_achIII_")
	...
	p_ach_savetime = register_cvar("exp_ach_savetime", "120.0")
	
	if(get_pcvar_float(p_ach_savetime) != 0.0)
		set_task(get_pcvar_float(p_ach_savetime), "Zaapisz", _, _, _, "b")
}
...
{
	if(get_playersnum() == get_cvar_num("exp_of_players"))
	{
		for(new i = 1 ; i < 33 ; i++)
		{
			if(is_user_connected(i) && i != id && !is_user_hltv(i))
				SprawdzAchs(i);
		}
	}
	
	set_task(3.5, "SprawdzAchs", id)
}
...
public MenuAch(id)
{
	...
	formatex(title, 255, "%L", id, "MENU_ACH_TITLE")
	new menu = menu_create(title, "WyborAchi_Wybieranie");
	new menu_cb = menu_makecallback("WyborAchi_Cb");
	for(new i = 1 ; i <= liczba_wszystkich_ach ; i++)
	{
		if(status_achI[id][i] == 0)
			formatex(opis, 255, "%s \r[%L]\y [%d lvl]", NazwaAchievementI[i], id, "MENU_ACH_LOCKED", WymaganyPoziomI[i]);
		...
		else if(status_achI[id][i] == 2)
		{
			if(status_achII[id][i] == 0)
				formatex(opis, 255, "%s \r[%L]\y [%d lvl]", NazwaAchievementII[i], id, "MENU_ACH_LOCKED",WymaganyPoziomII[i]);
			...
			else if(status_achII[id][i] == 2)
			{
				if(status_achIII[id][i] == 0)
					formatex(opis, 255, "%s \r[%L]\y [%d lvl]", NazwaAchievementIII[i], id, "MENU_ACH_LOCKED", WymaganyPoziomIII[i]);
					...
					formatex(opis, 255, "%s \r[%L]", NazwaAchievementIII[i], id, "MENU_ACH_COMPLETED")
			}
		}
...
public WyborAchi_Wybieranie(id, menu, item)
{
	...
	{
		formatex(tytul, 255, "\w%L: \r%s\w [\y%d\w/\y%d\w]^n%L:\r %s^n\w%L:\r %d\w %L", id, "NAME", NazwaAchievementI[item], postep_achI[id][item], PotrzebaI[item], id, "TARGET", OpisyAchievementI[item], id, "AWARD", NagrodaI[item], id, "EXP")
	}
	if(status_achI[id][item] == 2 && status_achII[id][item] == 1)
	{
		formatex(tytul, 255, "\w%L: \r%s\w [\y%d\w/\y%d\w]^n%L:\r %s^n\w%L:\r %d\w %L", id, "NAME", NazwaAchievementII[item], postep_achII[id][item], PotrzebaII[item], id, "TARGET", OpisyAchievementII[item], id, "AWARD", NagrodaII[item], id, "EXP")
	}
	if(status_achI[id][item] == 2 && status_achII[id][item] == 2 && status_achIII[id][item] == 1)
	{
		formatex(tytul, 255, "\w%L: \r%s\w [\y%d\w/\y%d\w]^n%L:\r %s^n\w%L:\r %d\w %L", id, "NAME", NazwaAchievementIII[item], postep_achIII[id][item], PotrzebaIII[item], id, "TARGET", OpisyAchievementIII[item], id, "AWARD", NagrodaIII[item], id, "EXP")
	}
...
public SprawdzAchs(id)
{
	if(!is_user_connected(id) || get_playersnum() < get_cvar_num("exp_of_players"))
		return PLUGIN_CONTINUE;
	...
	for(new i= 1; i <= liczba_wszystkich_ach ; i++)
	{
		...
		if(postep_achI[id][i] >= PotrzebaI[i] && status_achI[id][i] == 1)
		{
			...
			tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_I", id, "ACH_EARNED_ACH", NazwaAchievementI[i], OpisyAchievementI[i], id, "AWARD", NagrodaI[i])
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_I")
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementI[i])
			ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementI[i])
			ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaI[i])
		...
		else if(postep_achII[id][i] >= PotrzebaII[i] && status_achII[id][i] == 1)
		{
			...
			tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_II", id, "ACH_EARNED_ACH", NazwaAchievementII[i], OpisyAchievementII[i], id, "AWARD", NagrodaII[i])
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_II")
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementII[i])
			ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementII[i])
			ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaII[i])
		...
		else if(postep_achIII[id][i] >= PotrzebaIII[i] && status_achIII[id][i] == 1 )
		{
			...
			postep_achIII[id][i] = 0
			tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_III", id, "ACH_EARNED_ACH", NazwaAchievementIII[i], OpisyAchievementIII[i], id, "AWARD", NagrodaIII[i])
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_III")
			ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementIII[i])
			ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementIII[i])
			ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaIII[i])
...
public SprawdzAch(id, achid)
{
	if(!is_user_connected(id) || get_playersnum() < get_cvar_num("exp_of_players"))
		return PLUGIN_CONTINUE;
		
	if(exp_get_user_level(id) >= WymaganyPoziomI[achid] && status_achI[id][achid] == 0)
	{
		status_achI[id][achid] = 1;
		AchievementOdblokowany(id, achid, 1)
	}
	if(exp_get_user_level(id) >= WymaganyPoziomII[achid] && status_achII[id][achid] == 0 && status_achI[id][achid] == 2)
	{
		status_achII[id][achid] = 1;
		AchievementOdblokowany(id, achid, 2)
	}
			
	if(exp_get_user_level(id) >= WymaganyPoziomIII[achid] && status_achIII[id][achid] == 0 && status_achI[id][achid] == 2)
	{
		status_achIII[id][achid] = 1;
		AchievementOdblokowany(id, achid, 3)
	}
				
	if(postep_achI[id][achid] >= PotrzebaI[achid] && status_achI[id][achid] == 1)
	{
		status_achI[id][achid] = 2;
		postep_achI[id][achid] = 0
		postep_achII[id][achid] = 0
		
		tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_I", id, "ACH_EARNED_ACH", NazwaAchievementI[achid], OpisyAchievementI[achid], id, "AWARD", NagrodaI[achid])
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_I")
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementI[achid])
		ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementI[achid])
		ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaI[achid])
		AchievementZablokowany(id, achid, 1)
		exp_set_user_exp(id, exp_get_user_exp(id)+NagrodaI[achid]);
		exp_checklevel(id);
		return PLUGIN_CONTINUE
	}
	else if(postep_achII[id][achid] >= PotrzebaII[achid] && status_achII[id][achid] == 1)
	{
		status_achII[id][achid] = 2;
		postep_achII[id][achid] = 0
		postep_achIII[id][achid] = 0
	
		tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_II", id, "ACH_EARNED_ACH", NazwaAchievementII[achid], OpisyAchievementII[achid], id, "AWARD", NagrodaII[achid])
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_II")
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementII[achid])
		ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementII[achid])
		ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaII[achid])
		AchievementZablokowany(id, achid, 2)
		exp_set_user_exp(id, exp_get_user_exp(id)+NagrodaII[achid]);
		exp_checklevel(id);
		return PLUGIN_CONTINUE
	}
	else if(postep_achIII[id][achid] >= PotrzebaIII[achid] && status_achIII[id][achid] == 1 )
	{
		status_achIII[id][achid] = 2
		postep_achIII[id][achid] = 0
		tutorMake(id, TUTOR_GREEN, 5.0, "%L^n%L^n%s^n%L: %d", id, "ACH_LEVEL_III", id, "ACH_EARNED_ACH", NazwaAchievementIII[achid], OpisyAchievementIII[achid], id, "AWARD", NagrodaIII[achid])
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_LEVEL_III")	
		ColorChat(id, GREEN, "%s^x01 %L", prefix, id, "ACH_EARNED_ACH", NazwaAchievementIII[achid])
		ColorChat(id, GREEN, "%s^x01 %s", prefix, OpisyAchievementIII[achid])
		ColorChat(id, GREEN, "%s^x01 %L: %d", prefix, id, "AWARD", NagrodaIII[achid])
		AchievementZablokowany(id, achid, 3)
		exp_set_user_exp(id, exp_get_user_exp(id)+NagrodaIII[achid])
		exp_checklevel(id)
		return PLUGIN_CONTINUE
	}
	exp_checklevel(id)
...
public ZapiszI(id)
{
	...
	nvault_set(plikI, nick, dane[0])
	nvault_set(plikI_, nick, dane[1])
}
public ZapiszII(id)
{
	new nick[33], dane[2][500], szTemp[25]
	...
	nvault_set(plikII, nick, dane[0])
	nvault_set(plikII_, nick, dane[1])
}
public ZapiszIII(id)
{
	new nick[33], dane[2][500], szTemp[25]
	...
	nvault_set(plikIII, nick, dane[0])
	nvault_set(plikIII_, nick, dane[1])
}
...
public WczytajI(id)
{
	...
	nvault_get(plikI, nick, dane, 499)
	nvault_get(plikI_, nick, dane2, 499)
...
public WczytajII(id)
{
	new nick[33], dane[500], dane2[500]
	get_user_name(id, nick, 32)
	nvault_get(plikII, nick, dane, 499)
	nvault_get(plikII_, nick, dane2, 499)
...
public WczytajIII(id)
{
	new nick[33], dane[500], dane2[500]
	get_user_name(id, nick, 32)
	nvault_get(plikIII, nick, dane, 499)
	nvault_get(plikIII_, nick, dane2, 499)
...
public set_achievement_progress(index, achievementid, achievementlvl, value)
{
	...
	SprawdzAch(index, achievementid);
...
{
	if(value > 3 || value < 0 || get_playersnum() < get_cvar_num("exp_of_players"))
		return PLUGIN_CONTINUE
...
public plugin_end()
{
	...
	nvault_close(plikI_);
	nvault_close(plikII);
	nvault_close(plikII_);
	nvault_close(plikIII);
	nvault_close(plikIII_);
}
