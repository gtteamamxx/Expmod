...
public MenuSklep(id)
{
	...
	formatex(tytul, 127, "%L : ", LANG_PLAYER, "MENU_SHOP_TITLE", exp_get_user_monets(id))
...
public MenuSklep1(id, menu, item)
{
	...
	formatex(tytul, 255, "%L: \y%s^n\wO%L: \y%s^n\w%L: \y%d^n\w%L:\y %s^n\r%L ?", LANG_PLAYER, "NAME", NazwyItemow[item], LANG_PLAYER, "DESCRIPTION", OpisyItemow[item], LANG_PLAYER, "PRICE", KosztyItemow[item], LANG_PLAYER, "ONE_PER_ROUND", RazNaRunde[item] ? szYes:szNo, LANG_PLAYER, "BUY")	
...
public KupPrzedmiotForward(id, itemid)
{
	...
	ColorChat(id, GREEN, "%s^x01 %L:^x04 %s", prefix, LANG_PLAYER, "BOUGHT", NazwyItemow[itemid])
	ColorChat(id, GREEN, "%s^x01 %L:^x04 %s", prefix, LANG_PLAYER, "DESCRIPTION", OpisyItemow[itemid])
	ColorChat(id, GREEN, "%s^x01 %L:^x04 %d^x01 monet", prefix, LANG_PLAYER, "FOR", KosztyItemow[itemid])
	temp = 0;
