...
public cmd_anulujmisje(id, level, cid)
{
	...
	new missionid = exp_get_user_mission(player)
	if(exp_check_user_locked_mission(player, missionid) == 1)
	{
		ColorChat(id, GREEN, "[EXPMOD]^x01 Adminie :^x04 %s^x01! Gracz^x04 %s^x01 ma ta misje zablokowana!", nick_admina, nick_gracza)
		return PLUGIN_HANDLED
	}
	exp_unlock_or_lock_mission(player, missionid, 1)
	new nazwa_misji[64]
	exp_get_mission_name(missionid, nazwa_misji, 63)	
