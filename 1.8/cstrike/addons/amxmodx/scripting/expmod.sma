...
new g_MsgScoreAttrib = 0, gmsgStatusText;
new const POZIOM[] = 
{
...
public plugin_natives()
{
	...
	register_native("exp_get_user_mission_postep", 	"get_user_mission_postep", 	1);
	...
	register_native("get_loguser_index",		"get_loguser_index",			1);
...
public AnulujMisje_Wybieranie(id, menu, item)
{
	...
	if(item == 0)
	{
		...
		if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
			ColorChat(id, GREEN, "[EXPMOD]^x01 No trundo, zdecydowales sie anulowac misje, i zablokowac ja.")
...
public MenuAdmin(id)
{
	...
	menu_additem(menu, "Ustaw Monety")
	menu_additem(menu, "Dodaj Poziom")
	menu_additem(menu, "Dodaj Exp")
	menu_additem(menu, "Dodaj Monety")
...
public MenuAdminM_H(id)
{
	...
	for(new i = 1; i < 33; i++)
	{
		if(!is_user_hltv(i) && is_user_connected(i))
		{	
			get_user_name(i, nazwa_gracza, 32)
			formatex(gracz, 127, "%s [\y%d lvl\w]", nazwa_gracza, poziom_gracza[i])
...
public MenuAdminMg_Handler(id, menu, item)
{
	...
	new menu = menu_create("Wybierz misje^nX - Aktualna misja gracza", "MenuAdminM_Hh")
	new misja[64]
	admin_misja_wybor_gracz = item+1
	for(new i = 1 ; i < sizeof NazwaMisji ; i++)
	{
		if(i == misja_gracza[admin_misja_wybor_gracz])
			formatex(misja, 63, "%s \r[\y%d lvl\r] -\y X", NazwaMisji[i], WymaganyPoziom[i])
		else
...
public MenuAdminM_WH_CB(id, menu, item)
{
	...
	if(item == 1 && misja_gracza[admin_misja_wybor_gracz] != admin_misja)
		return ITEM_DISABLED
	
	if(item == 2 && misja_gracza[admin_misja_wybor_gracz] != admin_misja)
		return ITEM_DISABLED
	
	if(item == 3 && wykonane_misje[admin_misja_wybor_gracz][admin_misja] == 0)
		return ITEM_DISABLED
	
	if(item == 4 && wykonane_misje[admin_misja_wybor_gracz][admin_misja] == 1)
		return ITEM_DISABLED
...
public MenuAdminM_WH(id, menu, item)
{
	...
	if(item == 3)	
		ColorChat(id, GREEN, "[EXPMOD]^x01 Admin^x04 %s^x03 odblokowal^x01 misje^x04 %s^x01 graczowi^x04 %s", adminek, NazwaMisji[admin_misja], gracz)
	
	if(item == 4)
		ColorChat(id, GREEN, "[EXPMOD]^x01 Admin^x04 %s^x03 zablokowal^x01 misje^x04 %s^x01 graczowi^x04 %s", adminek, NazwaMisji[admin_misja], gracz)
	
	ZapiszMisje(admin_misja_wybor_gracz)
	return PLUGIN_CONTINUE
}
...
{
	new gracz[33]
	get_user_name(dodaj_exp_id, gracz, 32)
	
	new liczba_exp[10]
	read_args(liczba_exp, 9)
	remove_quotes(liczba_exp)
	
	if((doswiadczenie_gracza[dodaj_exp_id]+str_to_num(liczba_exp)) > POZIOM[199] || str_to_num(liczba_exp) <= 0)
		return PLUGIN_CONTINUE
	
	doswiadczenie_gracza[dodaj_exp_id] += str_to_num(liczba_exp)
	
	Zresetuj(dodaj_exp_id, 1, 1, 1)
...
{
	new menu = menu_create("Ustaw Monety graczowi : ", "MenuAdminUstawMonety_Handler")
	new gracz[128]
	for(new i = 1; i < 33; i++)
	{
		if(!is_user_hltv(i) && is_user_connected(i))
		{
			get_user_name(i, nazwa_gracza, 32)
			formatex(gracz,127, "%s \r[\y%d monet\r]", nazwa_gracza, monety_gracza[i])
			menu_additem(menu, gracz)
		}
	}
...
{
	new menu = menu_create("Dodaj Monety graczowi : ", "MenuAdminDodajMonety_Handler")
	new gracz[128]
...
public MenuAdminDodajMonety_Handler(id, menu, item)
{
	if(item == MENU_EXIT)
	{
		MenuAdmin(id)
...
{
	new gracz[33]
	get_user_name(dodaj_monety_id, gracz, 32)
	...
	monety_gracza[dodaj_monety_id] += str_to_num(liczba_monet)
...
public MenuAdminUstawMonety_Handler(id, menu, item)
{
	if(item == MENU_EXIT)
	{
		MenuAdmin(id)
...
{
	new gracz[33]
	get_user_name(ustaw_monety_id, gracz, 32)
	...
	monety_gracza[ustaw_monety_id] = str_to_num(liczba_monet)
...
{
	new gracz[33]
	get_user_name(ustaw_exp_id, gracz, 32)
	...
	doswiadczenie_gracza[ustaw_exp_id] = str_to_num(liczba_exp)
	
	Zresetuj(ustaw_exp_id, 1, 1, 1)
	if(str_to_num(liczba_exp) == 1)
...
{	
	...
	new gracz[33]
	get_user_name(ustaw_lvl_id, gracz, 32)
	...
	poziom_gracza[ustaw_lvl_id] = str
	doswiadczenie_gracza[ustaw_lvl_id] = POZIOM[str-1]
	
	ultra_punkty_gracza[ustaw_lvl_id] = floatround(float(poziom_gracza[ustaw_lvl_id]/10), floatround_floor)
	niewidzialnosc_gracza[ustaw_lvl_id] = 0
	nowa_niewidzialnosc_gracza[ustaw_lvl_id] = 0
	
	obrazenia_gracza[ustaw_lvl_id] = 0
	nowe_obrazenia_gracza[ustaw_lvl_id] = 0
	
	skoki_gracza[ustaw_lvl_id] = 0
	nowe_skoki_gracza[ustaw_lvl_id] = 0
	
	longjump_gracza[ustaw_lvl_id] = 0
	nowy_longujmp_gracza[ustaw_lvl_id] = 0
	longjump_s_czas[ustaw_lvl_id] = 0
	
	punkty_gracza[ustaw_lvl_id] = poziom_gracza[ustaw_lvl_id]*PUNKTY_ZA_LEVEL
	
	zycie_gracza[ustaw_lvl_id] = 0
	nowe_zycie_gracza[ustaw_lvl_id] = 0
	
	szybkosc_gracza[ustaw_lvl_id] = 0
	nowa_szybkosc_gracza[ustaw_lvl_id] = 0.0
	
	grawitacja_gracza[ustaw_lvl_id] = 0
	nowa_grawitacja_gracza[ustaw_lvl_id] = 0.0
	
	kasa_gracza[ustaw_lvl_id] = 0
	nowa_kasa_gracza[ustaw_lvl_id] = 0
	monety_gracza[dodaj_exp_id] = 0
...
public DeathMsg()
{
	...
	if(atakujacy != obronca && is_user_connected(atakujacy) && get_user_team(atakujacy) != get_user_team(obronca))
	{
		...
		{	
			...
			{
				if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
					ColorChat(atakujacy, GREEN, "[EXPMOD]^x01 Musisz zabic z noza^x04 %d^x01 wrogow", (PotrzebaMisji[misja_gracza[atakujacy]]-postep[atakujacy]))
				...
				if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
				{
					...
					tutorMake(atakujacy, TUTOR_GREEN, 5.0, "Musisz zabic z noza %d wrogow", (PotrzebaMisji[misja_gracza[atakujacy]]-postep[atakujacy]))
				}
			}
		}
...
public Pokaz(id)
{
	...
	for(new i = 1 ; i < 33 ; i++)
	{
		...
		if(!is_user_alive(i) && is_user_connected(i) && !is_user_bot(i))
		{
			...
			ShowSyncHudMsg(i, Message4, "EXP : %i / %i | %0.1f%%^nPoziom : %i  ^nZdrowie : %i^nMisja : %s", doswiadczenie_gracza[cel], POZIOM[poziom_gracza[cel]+1], dos_cel, poziom_gracza[cel], get_user_health(cel), NazwaMisji[misja_gracza[cel]])
		...
		else if(is_user_alive(i) && is_user_connected(i) && !is_user_bot(i))
		{
			...
			formatex(misja, charsmax(misja), "Misja: %s | Postep: %d/%d | Nagroda: %d", NazwaMisji[misja_gracza[i]], postep[i], PotrzebaMisji[misja_gracza[i]], NagrodaMisji[misja_gracza[i]])
...
public WyborMisji(id)
{
	...
	for(new i = 1 ; i < sizeof NazwaMisji ; i++)
	{
		formatex(opis, 255, "%s \w[\y%d lvl\w] [\r%d exp\w]",NazwaMisji[i], WymaganyPoziom[i], NagrodaMisji[i])
...
public get_mission_name(mission_number, dest[], len)
{
	param_convert(2)
	copy(dest, len, NazwaMisji[mission_number])
}
public get_mission_description(mission_number, dest[], len)
{
	param_convert(2)
	copy(dest,len, OpisMisji[mission_number])
}
...
public get_user_mission_postep(index)
{
	return postep[index]
}
public plugin_end()
