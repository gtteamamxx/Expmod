...
new OpisyAchiI[][] =
{
	"Zabij z pistoleta 100 wrogow",  //0
	"Zabij z szotgana 150 wrogow", 	 //1
	"Zabij z SMG 250 wrogow", 	 //2
	"Zabij z Rifle 300 wrogow", 	 //3
	"Zabij z M249 200 wrogow",  	 //4
	"Podloz 8 bomb",		 //5
	"Rozbroj 8 bomb",		 //6
	"Zabij 150 wrogow ze snajperki", //7
	"Zabij 50 wrogow z noza",	 //8
	"Zabij headshotem 100 wrogow",	 //9
	"Zabij kleczac 150 wrogow",	 //10
	"Zabij 500 wrogow",		 //11
	"Zabij w powietrzu 7 wrogow",	 //12
	"Zadaj w sumie 15tys. obrazen"	 //13
//	"Uratuj 35 zakladnikow"		 //14
}

new OpisyAchiII[][] =
{
	"Zabij z pistoleta 300 wrogow",	 //0
	"Zabij z szotgana 360 wrogow",	 //1
	"Zabij z SMG 620 wrogow",	 //2
	"Zabij z Rifle 800 wrogow",	 //3
	"Zabij z M249 450 wrogow",	 //4
	"Podloz 20 bomb",		 //5
	"Rozbroj 20 bomb",		 //6
	"Zabij 300 wrogow ze snajperki", //7
	"Zabij 100 wrogow z noza",	 //8
	"Zaboj heashotem 250 wrogow",	 //9
	"Zabij kleczac 300 wrogow", 	 //10
	"Zabij 1200 wrogow",		 //11
	"Zabij w powietrzu 15 wrogow",	 //12
	"Zadaj w sumie 50tys. obrazen"	 //13
//	"Uratuj 80 zakladnikow"	 //14
}

new OpisyAchiIII[][] = 
{
	"Zabij z pistoleta 650 wrogow",	 //0
	"Zabij z szotgana 600 wrogow",	 //1
	"Zabij z SMG 1000 wrogow",	 //2
	"Zabij z Rifle 1300 wrogow",	 //3
	"Zabij z M249 800 wrogow",	 //4
	"Podloz 50 bomb",		 //5
	"Rozbroj 50 bomb",  		 //6
	"Zabij 500 wrogow ze snajperki", //7
	"Zabij 300 wrogow z noza",	 //8
	"Zabij headshotem 600 wrogow",	 //9
	"Zabij kleczac 700 wrogow",	 //10
	"Zabij 3000 wrogow",		 //11
	"Zabij w powietrzu 30 wrogow",	 //12
	"Zadaj w sumie 150tys. obrazen"	 //13
//	"Uratuj 150 zakladnikow"	 //14
}
...
new ach_unlock1[]	= "ExpMod/ach_unlock.wav"
new ach_unlock2[]	= "ExpMod/ach_unlock1.wav"
...
public WyborAchi(id)
{
	...
	for(new i = 0 ; i < liczba_ach ; i++)
	{
		if(status_achI[id][i] == 0)
			formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwyAchiI[i], WymaganeLevelI[i]);
		else if(status_achI[id][i] == 1)
			formatex(opis, 255, "%s \r[%d / %d]\y [%d exp]", NazwyAchiI[i], postep_achI[id][i], PotrzebyAchI[i], NagrodyAchiI[i]);
		else if(status_achI[id][i] == 2)
		{
			if(status_achII[id][i] == 0)
				formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwyAchiII[i], WymaganeLevelII[i]);
			else if(status_achII[id][i] == 1)
				formatex(opis, 255, "%s \r[%d / %d]\y [%d exp]", NazwyAchiII[i], postep_achII[id][i], PotrzebyAchII[i], NagrodyAchiII[i]);
			else if(status_achII[id][i] == 2)
			{
				if(status_achIII[id][i] == 0)
					formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwyAchiIII[i], WymaganeLevelIII[i]);
				else if(status_achIII[id][i] == 1)
					formatex(opis, 255, "%s \r[%d / %d]\y [%d exp]", NazwyAchiIII[i], postep_achIII[id][i], PotrzebyAchIII[i], NagrodyAchiIII[i]);
				else if(status_achIII[id][i] == 2)
					formatex(opis, 255, "%s \r[Ukonczony]", NazwyAchiIII[i])
			}
		}
...
/*public UratowanieHosta()
{
	...
	if(status_achI[id][14] == 1)
		postep_achI[id][14]++
	else if(status_achII[id][14] == 1)
		postep_achII[id][14]++
	else if(status_achIII[id][14] == 1)
		postep_achIII[id][14]++
}*/
...
public SprawdzAch(id)
{
	...
	for(new i= 0; i < liczba_ach ; i++)
	{
		...
		else if(postep_achII[id][i] >= PotrzebyAchII[i] && status_achII[id][i] == 1)
		{
			status_achII[id][i] = 2;
			postep_achII[id][i] = 0
		...
		else if(postep_achIII[id][i] >= PotrzebyAchIII[i] && status_achIII[id][i] == 1 )
		{
...
public WybierzOpisAchi_Handler(id, menu, item)
{
	...
	new opis[512]
	if(item == 0)
		formatex(opis, 511, "\yAchievement : \r%s^n\yZadanie : \r%s^n\yNagroda :\r %d\y expa^n\yWymagany Lvl : \r%d", NazwyAchiI[opis_achitem[id]], OpisyAchiI[opis_achitem[id]], NagrodyAchiI[opis_achitem[id]], WymaganeLevelI[opis_achitem[id]])
	if(item == 1)
		formatex(opis, 511, "\yAchievement : \r%s^n\yZadanie : \r%s^n\yNagroda :\r %d\y expa^n\yWymagany Lvl : \r%d", NazwyAchiII[opis_achitem[id]], OpisyAchiII[opis_achitem[id]], NagrodyAchiII[opis_achitem[id]], WymaganeLevelII[opis_achitem[id]])
	if(item == 2)
		formatex(opis, 511, "\yAchievement : \r%s^n\yZadanie : \r%s^n\yNagroda :\r %d\y expa^n\yWymagany Lvl : \r%d", NazwyAchiIII[opis_achitem[id]], OpisyAchiIII[opis_achitem[id]], NagrodyAchiIII[opis_achitem[id]], WymaganeLevelIII[opis_achitem[id]])
	if(item == 3)
		MenuOpisAchi(id)
...
public WczytajAchI(id)
{
	...
	new sg[sizeof NazwyAchiI][16] // sg - Status Gracza
	new pg[sizeof NazwyAchiI][16] // pg - Postep Gracza
...
public WczytajAchII(id)
{
	...
	new sg[sizeof NazwyAchiII][16]
	new pg[sizeof NazwyAchiII][16]
...
public WczytajAchIII(id)
{
	...
	new sg[sizeof NazwyAchiIII][16]
	new pg[sizeof NazwyAchiIII][16]
