...
public ustaw_monety()
{
	...
	if(str_to_num(liczba_monet) < 0)
	{
		ColorChat(get_user_index(adminek), GREEN, "%s^x01 %L", prefix, "MSG_ADMIN_TOO_LOW_ERROR")
		return PLUGIN_CONTINUE
	}
	...
	ColorChat(0, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_ADMIN_SET_MONETS", adminek, str_to_num(liczba_monet), gracz)
...
public ustaw_exp()
{
	...
	ColorChat(0, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_ADMIN_SET_EXP", adminek, str_to_num(liczba_exp), gracz)
...
{	
	...
	Zresetuj(id_am, 1, 1, 1)
	ColorChat(0, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_ADMIN_SET_LEVEL", adminek, str, gracz)
...
public SprawdzExp(id)
{
	...
	while(doswiadczenie_gracza[id] >= POZIOM[poziom_gracza[id]+1])
	{
		...
		ShowSyncHudMsg(id, Message2,  "%L", LANG_PLAYER, "MSG_LEVEL_UP", get_user_level(id))
		
		if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
		{
			ColorChat(id, GREEN, "%s^x01 %L.", prefix, LANG_PLAYER, "MSG_LEVEL_UP", get_user_level(id))
		}
		...
		if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
		{
			...
			tutorMake(id, TUTOR_YELLOW, 5.0, "%L", LANG_PLAYER, "MSG_LEVEL_UP", poziom_gracza[id])
		}
		...
		{
			if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
				ColorChat(0, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_HIGH_LEVEL_EARNED", nazwa_gracza, poziom_gracza[id])
			...
			if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
			{
				...
				tutorMake(0, TUTOR_YELLOW, 5.0, "%L", LANG_PLAYER, "MSG_HIGH_LEVEL_EARNED", nazwa_gracza, poziom_gracza[id])
			}
		}
...
public Pokaz(id)
{
	...
	for(new i = 1 ; i < 33 ; i++)
	{
		...
		if(!is_user_alive(i) && is_user_connected(i) && !is_user_bot(i))
		{
			...
			ShowSyncHudMsg(i, Message4, "%L : %i / %i | %0.1f%%^n%L : %i  ^n%L : %i", LANG_PLAYER, "EXP", doswiadczenie_gracza[cel], POZIOM[poziom_gracza[cel]+1], dos_cel, LANG_PLAYER, "LEVEL", poziom_gracza[cel],  LANG_PLAYER, "HEALTH", get_user_health(cel))
		...
		else if(is_user_alive(i) && is_user_connected(i) && !is_user_bot(i))
		{
			...
			ShowSyncHudMsg(i, Message4, "[%L : %i / %i | %0.1f%%]^n[%L : %i]^n[%L : %i]^n[%L : %i]", LANG_PLAYER, "EXP", doswiadczenie_gracza[i], POZIOM[poziom_gracza[i]+1], dos,  LANG_PLAYER, "LEVEL", poziom_gracza[i], LANG_PLAYER, "HEALTH", get_user_health(i), LANG_PLAYER, "MONETS", monety_gracza[i])
...
public bomb_planted(podkladajacy)
{
	...
	if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
		ColorChat(podkladajacy, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_BOMB_PLANTED", get_pcvar_num(p_exp_podlozenie_paki))
	
	if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
		tutorMake(podkladajacy, TUTOR_GREEN, 5.0, "%L", LANG_PLAYER, "MSG_BOMB_PLANTED", get_pcvar_num(p_exp_podlozenie_paki))
...
public bomb_defused(rozbrajajacy)
{
	...
	if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
		ColorChat(rozbrajajacy, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_DEFUSED_BOMB", get_pcvar_num(p_exp_rozbrojenie_paki))
	...
	if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
	{
		...
		tutorMake(rozbrajajacy, TUTOR_GREEN, 5.0, "%L", LANG_PLAYER, "MSG_DEFUSED_BOMB", get_pcvar_num(p_exp_rozbrojenie_paki))
	}
...
public UratowanieHosta()
{
	...
	if(get_pcvar_num(p_exp_infostyle) == 2 || get_pcvar_num(p_exp_infostyle) == 3)
		ColorChat(id, GREEN, "%s^x01 %L", prefix, LANG_PLAYER, "MSG_RESCUED_HOSTAGE", get_pcvar_num(p_exp_uratowanie_hosta))
	...
	if(get_pcvar_num(p_exp_infostyle) == 1 || get_pcvar_num(p_exp_infostyle) == 3)
	{
		...
		tutorMake(id, TUTOR_GREEN, 5.0, "%L", LANG_PLAYER, "MSG_RESCUED_HOSTAGE", get_pcvar_num(p_exp_uratowanie_hosta))
	}
