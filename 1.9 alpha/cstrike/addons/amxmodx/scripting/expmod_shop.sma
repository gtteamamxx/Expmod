...
public MenuSklep(id)
{
	...
	formatex(tytul, 127, "Masz \y%d\w monet^n\yCo chcesz kupic ? : ", exp_get_user_monets(id))
...
public MenuSklep1(id, menu, item)
{
	...
	temp = item;
	new tytul[256]
	formatex(tytul, 255, "Nazwa: \y%s^n\wOpis: \y%s^n\wCena: \y%d^n\wRaz na runde:\y %s^n\rKupujesz ?", NazwyItemow[item], OpisyItemow[item], KosztyItemow[item], RazNaRunde[item] ? "Tak":"Nie")	
...
public KupPrzedmiotForward(id, itemid)
{
	...
	ColorChat(id, GREEN, "%s^x01 Kupiles:^x04 %s", prefix, NazwyItemow[itemid])
	ColorChat(id, GREEN, "%s^x01 Opis:^x04 %s", prefix, OpisyItemow[itemid])
	ColorChat(id, GREEN, "%s^x01 Za:^x04 %d^x01 monet", prefix, KosztyItemow[itemid])
	temp = 0;
