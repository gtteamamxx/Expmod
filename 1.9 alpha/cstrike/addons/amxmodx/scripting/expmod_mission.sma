...
public OpisMisji(id)
{	
	new menu = menu_create("Wybierz Misje : ", "OpisMisji1")
...
public OpisMisji1(id, menu, item) {
	...
	formatex(szOpis, charsmax(szOpis), "\yMisja :\w %s^n\yOpis :\w %s^n\yWymagany lvl :\r %d^n\yNagroda :\r %d\y expa", NazwyMisji[item], OpisyMisji[item], WymaganyPoziom[item], NagrodaMisji[item])
...
public AnulujMisje(id)
{
	...
	{
		new tytul[256]
		formatex(tytul, 255, "\yMisja :\r %s^n\yZadanie : \r%s^n\yPostep : \r%d\w /\r %d^n\rNaprawde chcesz anulowac misje?",NazwyMisji[misja_gracza[id]], OpisyMisji[misja_gracza[id]], PostepGracza[id], PotrzebaMisji[misja_gracza[id]])
	...
	{
		ColorChat(id, GREEN, "%s^x01 Nie masz zadnej misji!", prefix)
		return PLUGIN_CONTINUE
	}
	return PLUGIN_CONTINUE
}
public AnulujMisjeH(id, menu, item)
{
	...
	if(item == 0)
	{
		ColorChat(id,GREEN,"%s^x01 Zdecydowales sie^x03 anulowac misje :^x04 %s", prefix, NazwyMisji[misja_gracza[id]])
...
public MenuMisji(id)
{
	new menu = menu_create("Wybierz Misje : ", "MenuMisjiH")
	new opis[85]
...
public MenuMisjiH(id, menu, item)
{
	...
	temp = item;
	new misja[256]
	formatex(misja, 255, "\yMisja :\w %s^n\yZadanie :\w %s^n\yWymagany Lvl :\w %d^n\yNagroda :\w %d expa", NazwyMisji[item], OpisyMisji[item], WymaganyPoziom[item], NagrodaMisji[item])
...
public MenuMisji2H(id, menu, item)
{
	...
	if(item == 0)
	{
		...
		ColorChat(id, GREEN, "%s^x01 Wybrales Misje:^x04 %s", prefix, NazwyMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x01 Zadanie:^x04 %s", prefix, OpisyMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x01 Nagroda:^x04 %d expa", prefix, NagrodaMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x03 Powodzenia^x01 !", prefix)
	}
	return PLUGIN_CONTINUE
}
...
public SprawdzMisje(id)
{
	...
	{
		ColorChat(id, GREEN, "%s^x03 Gratulacje!^x01 Ukonczyles Misje :^x04 %s", prefix, NazwyMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x01 Zadanie :^x04 %s", prefix, OpisyMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x01 Nagroda :^x04 %i expa", prefix, NagrodaMisji[misja_gracza[id]])
...
public PokazMisje()
{
	for(new i = 1 ; i < 33 ; i++)
	{
		...
		else if(is_user_alive(i) && is_user_connected(i) && !is_user_bot(i))
		{
			...
			formatex(misja, charsmax(misja), "Misja: %s | Postep: %d/%d (+%d)", NazwyMisji[misja_gracza[i]], PostepGracza[i], PotrzebaMisji[misja_gracza[i]], PotrzebaMisji[misja_gracza[i]]-PostepGracza[i])
