...
public MenuAch(id)
{
	...
	for(new i = 1 ; i <= liczba_wszystkich_ach ; i++)
	{
		if(status_achI[id][i] == 0)
			formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwaAchievementI[i], WymaganyPoziomI[i]);
		...
		else if(status_achI[id][i] == 2)
		{
			if(status_achII[id][i] == 0)
				formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwaAchievementII[i], WymaganyPoziomII[i]);
			...
			else if(status_achII[id][i] == 2)
			{
				if(status_achIII[id][i] == 0)
					formatex(opis, 255, "%s \r[Zablokowany]\y [%d lvl]", NazwaAchievementIII[i], WymaganyPoziomIII[i]);
					...
					formatex(opis, 255, "%s \r[Ukonczony]", NazwaAchievementIII[i])
			}
		}
...
public WyborAchi_Wybieranie(id, menu, item)
{
	...
	{
		formatex(tytul, 255, "\wNazwa: \r%s\w [\y%d\w/\y%d\w]^nCel:\r %s^n\wNagroda:\r %d\w expa", NazwaAchievementI[item], postep_achI[id][item], PotrzebaI[item], OpisyAchievementI[item], NagrodaI[item])
	}
	if(status_achI[id][item] == 2 && status_achII[id][item] == 1)
	{
		formatex(tytul, 255, "\wNazwa: \r%s\w [\y%d\w/\y%d\w]^nCel:\r %s^n\wNagroda:\r %d\w expa", NazwaAchievementII[item], postep_achII[id][item], PotrzebaII[item], OpisyAchievementII[item], NagrodaII[item])
	}
	if(status_achI[id][item] == 2 && status_achII[id][item] == 2 && status_achIII[id][item] == 1)
	{
		formatex(tytul, 255, "\wNazwa: \r%s\w [\y%d\w/\y%d\w]^nCel:\r %s^n\wNagroda:\r %d\w expa", NazwaAchievementIII[item], postep_achIII[id][item], PotrzebaIII[item], OpisyAchievementIII[item], NagrodaIII[item])
	}
...
public SprawdzAch(id)
{
	...
	for(new i= 1; i <= liczba_wszystkich_ach ; i++)
	{
		...
		if(postep_achI[id][i] >= PotrzebaI[i] && status_achI[id][i] == 1)
		{
			...
			tutorMake(id, TUTOR_GREEN, 5.0, "Achievmenty Poziom I^nZdobyles Osiagniecie %s^n%s^nNagroda : %d", NazwaAchievementI[i],OpisyAchievementI[i],NagrodaI[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Achievmenty Poziom^x04 I")
			ColorChat(id, GREEN, "[EXPMOD]^x01 Zdobyles Osiagniecie :^x04 %s", NazwaAchievementI[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 %s", OpisyAchievementI[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Nagroda :^x04 %d", NagrodaI[i])
		...
		else if(postep_achII[id][i] >= PotrzebaII[i] && status_achII[id][i] == 1)
		{
			...
			tutorMake(id, TUTOR_GREEN, 5.0, "Achievmenty Poziom II^nZdobyles Osiagniecie %s^n%s^nNagroda : %d", NazwaAchievementII[i],OpisyAchievementII[i],NagrodaII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Achievmenty Poziom^x04 II")
			ColorChat(id, GREEN, "[EXPMOD]^x01 Zdobyles Osiagniecie :^x04 %s", NazwaAchievementII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 %s", OpisyAchievementII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Nagroda :^x04 %d", NagrodaII[i])
		...
		else if(postep_achIII[id][i] >= PotrzebaIII[i] && status_achIII[id][i] == 1 )
		{
			...
			postep_achIII[id][i] = 0
			tutorMake(id, TUTOR_GREEN, 5.0, "Achievmenty Poziom III^nZdobyles Osiagniecie %s^n%s^nNagroda : %d", NazwaAchievementIII[i],OpisyAchievementIII[i],NagrodaIII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Achievmenty Poziom^x04 III")
			ColorChat(id, GREEN, "[EXPMOD]^x01 Zdobyles Osiagniecie :^x04 %s", NazwaAchievementIII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 %s", OpisyAchievementIII[i])
			ColorChat(id, GREEN, "[EXPMOD]^x01 Nagroda :^x04 %d", NagrodaIII[i])
