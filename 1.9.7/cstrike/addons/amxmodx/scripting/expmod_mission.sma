...
public MenuMisji2H(id, menu, item)
{
	...
	if(item == 0)
	{
		...
		ColorChat(id, GREEN, "%s^x01 %L:^x04 %s", prefix, LANG_PLAYER, "TARGET", OpisyMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x01 %L:^x04 %d exp", prefix, LANG_PLAYER, "AWARD", NagrodaMisji[misja_gracza[id]])
		ColorChat(id, GREEN, "%s^x03 %L^x01 !", prefix, LANG_PLAYER, "GOOD_LUCK")
	}
	return PLUGIN_CONTINUE
}
