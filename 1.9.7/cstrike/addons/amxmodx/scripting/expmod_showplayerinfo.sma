...
public Show(id)
{
	...
	if(get_user_team(cel) == get_user_team(id))
	{
		...
		if(get_pcvar_num(team_show_nick) == 1)
			Len += formatex(txt, (sizeof txt - 1) - Len, "%L: %s^n", LANG_PLAYER, "NAME", nazwa_gracza)
		if(get_pcvar_num(team_show_health) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", LANG_PLAYER, "HEALTH", get_user_health(cel))
		if(get_pcvar_num(team_show_level) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", LANG_PLAYER, "LEVEL", exp_get_user_level(cel))
		if(get_pcvar_num(team_show_exp) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %d^n", LANG_PLAYER, "EXP", exp_get_user_exp(cel))
		if(get_pcvar_num(team_show_mission) == 1)
			Len += formatex(txt[Len], (sizeof txt - 1) - Len, "%L: %s^n", LANG_PLAYER, "MISSION", nazwa_misji)
